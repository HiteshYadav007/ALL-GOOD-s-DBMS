...
export const getProducts =async (
	...
	if(subCategoryId !== undefined && sizeId !== undefined && isFeatured !== undefined){
		...
		queryString = 'Select store.product.productId , store.product.name , store.product.price ,store.product.categoryId , store.product.quantity , store.product.isFeatured , store.product.imageUrl ,store.product.subCategoryId,store.subCategory.name as subCategoryName , store.size.name as sizeName from store.product INNER JOIN store.subcategory ON store.product.subCategoryId = store.subcategory.subCategoryId INNER JOIN store.size on store.product.sizeId = store.size.sizeId where store.product.categoryId = ?;'
...
export const getProductsByCategory = async (
	...
	const queryString = 'Select * from store.product where categoryId = ?';
